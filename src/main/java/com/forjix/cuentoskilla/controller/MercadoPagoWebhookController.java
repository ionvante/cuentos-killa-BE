...
public class MercadoPagoWebhookController {
    ...
    public ResponseEntity<?> handleWebhook(@RequestBody(required = false) Map<String, Object> notificationPayload,
        ...
        if (notificationPayload == null || notificationPayload.isEmpty()) {
            if (allRequestParams.containsKey("id") && allRequestParams.containsKey("topic")) { // Common for
                                                                                               // test/validation
                logger.info("Received MP test/validation notification. ID: {}, Topic: {}", allRequestParams.get("id"),
                        allRequestParams.get("topic"));
                return ResponseEntity.ok().build();
            }
            // It could also be a notification type that only sends query parameters (though
            // less common for 'payment')
            if (allRequestParams.containsKey("type") && "payment".equals(allRequestParams.get("type"))
                ...
                logger.info("Payment notification received via query parameters for Mercado Pago Payment ID: {}",
                        paymentId);
                ...
                try {
                    ...
                    if (payment != null && "approved".equals(payment.getStatus())) {
                        ...
                        if (externalReference != null && !externalReference.isEmpty()) {
                            ...
                            logger.info("Order {} marked as PAID via webhook parameter.", externalReference);
                        }
                    }
                } catch (Exception e) {
                    logger.error("Error processing query param webhook for payment ID {}: {}", paymentId,
                            e.getMessage());
                }
            ...
            logger.warn(
                    "Received empty or null webhook payload and not recognized as a test/validation or query param based notification.");
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Map.of("error", "Empty or unrecognized payload/parameters"));
        }
                ...
                if (data.containsKey("id")) {
                    ...
                    try {
                        ...
                        if (payment != null && "approved".equals(payment.getStatus())) {
                            ...
                            if (externalReference != null && !externalReference.isEmpty()) {
                                orderService.processWebhookPayment(Long.valueOf(externalReference));
                                logger.info("Order {} marked as PAID via JSON webhook payload.", externalReference);
                            }
                        }
                    } catch (Exception e) {
                        logger.error("Error processing JSON webhook for payment ID {}: {}", paymentId, e.getMessage());
                    }
                    ...
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body(Map.of("error", "Missing payment ID in data"));
                }
            } else {
                logger.warn("Payment notification 'data' field is not a Map or is missing. Payload: {}",
                        notificationPayload);
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(Map.of("error", "Invalid 'data' field in payment notification"));
            }
        ...
        // Acknowledge receipt even if not fully processed to prevent MP from resending
        // unnecessarily for unhandled types
